...
class CartesianPoseRightArmWaistServer : public rclcpp::Node
{
public:
  using SetPose = sciurus17_interfaces::action::SetPose;
  using GoalHandleSetPose = rclcpp_action::ServerGoalHandle<SetPose>;

  CartesianPoseRightArmWaistServer(const rclcpp::Node::SharedPtr & move_group_node) : Node("cartesian_pose_right_arm_waist_node"), move_group_node_(move_group_node)
  {
    action_server_ = rclcpp_action::create_server<SetPose>(
      this,
      "cartesian_pose_right_arm_waist",  // アクション名
      std::bind(&CartesianPoseRightArmWaistServer::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
      std::bind(&CartesianPoseRightArmWaistServer::handle_cancel, this, std::placeholders::_1),
      std::bind(&CartesianPoseRightArmWaistServer::handle_accepted, this, std::placeholders::_1)
    );
...
private:
  ...
  void handle_accepted(const std::shared_ptr<GoalHandleSetPose> goal_handle)
  {
    ...
    geometry_msgs::msg::Pose pose_start = move_group.getCurrentPose("r_link7").pose;
...
int main(int argc, char **argv)
{
  ...
  auto action_node = std::make_shared<CartesianPoseRightArmWaistServer>(move_group_node);
