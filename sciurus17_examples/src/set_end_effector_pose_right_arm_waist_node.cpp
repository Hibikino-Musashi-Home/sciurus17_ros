...
class SetEndEffectorPoseRightArmWaistServer : public rclcpp::Node
{
public:
  using SetPose = sciurus17_interfaces::action::SetPose;
  using GoalHandleSetPose = rclcpp_action::ServerGoalHandle<SetPose>;

  SetEndEffectorPoseRightArmWaistServer(const rclcpp::Node::SharedPtr & move_group_node) : Node("set_end_effector_pose_right_arm_waist_node"), move_group_node_(move_group_node)
  {
    action_server_ = rclcpp_action::create_server<SetPose>(
      this,
      "set_end_effector_pose_right_arm_waist",  // アクション名
      std::bind(&SetEndEffectorPoseRightArmWaistServer::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
      std::bind(&SetEndEffectorPoseRightArmWaistServer::handle_cancel, this, std::placeholders::_1),
      std::bind(&SetEndEffectorPoseRightArmWaistServer::handle_accepted, this, std::placeholders::_1)
    );
...
private:
  ...
  std::thread([this, goal_handle]() {
    ...
    // ゴール姿勢をセット
    move_group.setPoseTarget(pose, "r_link7");
...
int main(int argc, char **argv)
{
  ...
  auto action_node = std::make_shared<SetEndEffectorPoseRightArmWaistServer>(move_group_node);
