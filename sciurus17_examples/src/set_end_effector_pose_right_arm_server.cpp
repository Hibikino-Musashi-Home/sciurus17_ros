...
class SetEndEffectorPoseRightArmServer : public rclcpp::Node
{
public:
  using SetPose = sciurus17_interfaces::action::SetPose;
  using GoalHandleSetPose = rclcpp_action::ServerGoalHandle<SetPose>;

  SetEndEffectorPoseRightArmServer(const rclcpp::Node::SharedPtr & move_group_node) : Node("set_end_effector_pose_right_arm_node"), move_group_node_(move_group_node)
  {
    action_server_ = rclcpp_action::create_server<SetPose>(
      this,
      "set_end_effector_pose_right_arm",  // アクション名
      std::bind(&SetEndEffectorPoseRightArmServer::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
      std::bind(&SetEndEffectorPoseRightArmServer::handle_cancel, this, std::placeholders::_1),
      std::bind(&SetEndEffectorPoseRightArmServer::handle_accepted, this, std::placeholders::_1)
    );
...
private:
  ...
  std::thread([this, goal_handle]() {
    ...
    moveit::planning_interface::MoveGroupInterface move_group(move_group_node_, "r_arm_group");
...
int main(int argc, char **argv)
{
  ...
  auto action_node = std::make_shared<SetEndEffectorPoseRightArmServer>(move_group_node);
