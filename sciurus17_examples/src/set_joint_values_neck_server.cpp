...
class SetJointValuesNeckServer : public rclcpp::Node, public std::enable_shared_from_this<SetJointValuesNeckServer>
{
public:
  ...
  SetJointValuesNeckServer(const rclcpp::Node::SharedPtr & move_group_node) : Node("set_joint_values_neck_server"), move_group_node_(move_group_node)
  {
    action_server_ = rclcpp_action::create_server<SetJointValues>(
      this,
      "set_joint_values_neck",
      std::bind(&SetJointValuesNeckServer::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
      std::bind(&SetJointValuesNeckServer::handle_cancel, this, std::placeholders::_1),
      std::bind(&SetJointValuesNeckServer::handle_accepted, this, std::placeholders::_1)
    );

    RCLCPP_INFO(this->get_logger(), "SetJointValuesNeckServer is up.");
  }
...
private:
  ...
  std::thread([this, goal_handle]() {
    ...
    moveit::planning_interface::MoveGroupInterface move_group(move_group_node_, "neck_group");
...
int main(int argc, char **argv)
{
  ...
  auto action_node = std::make_shared<SetJointValuesNeckServer>(move_group_node);
