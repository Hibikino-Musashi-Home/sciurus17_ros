...
class SetEndEffectorPoseLeftArmWaistServer : public rclcpp::Node
{
public:
  using SetPose = sciurus17_interfaces::action::SetPose;
  using GoalHandleSetPose = rclcpp_action::ServerGoalHandle<SetPose>;

  SetEndEffectorPoseLeftArmWaistServer(const rclcpp::Node::SharedPtr & move_group_node) : Node("set_end_effector_pose_left_arm_waist_node"), move_group_node_(move_group_node)
  {
    action_server_ = rclcpp_action::create_server<SetPose>(
      this,
      "set_end_effector_pose_left_arm_waist",  // アクション名
      std::bind(&SetEndEffectorPoseLeftArmWaistServer::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
      std::bind(&SetEndEffectorPoseLeftArmWaistServer::handle_cancel, this, std::placeholders::_1),
      std::bind(&SetEndEffectorPoseLeftArmWaistServer::handle_accepted, this, std::placeholders::_1)
    );
...
int main(int argc, char **argv)
{
  ...
  auto action_node = std::make_shared<SetEndEffectorPoseLeftArmWaistServer>(move_group_node);
