...
class SetJointValuesRightArmWaistServer : public rclcpp::Node, public std::enable_shared_from_this<SetJointValuesRightArmWaistServer>
{
public:
  ...
  SetJointValuesRightArmWaistServer(const rclcpp::Node::SharedPtr & move_group_node) : Node("set_joint_values_right_arm_waist_server"), move_group_node_(move_group_node)
  {
    action_server_ = rclcpp_action::create_server<SetJointValues>(
      this,
      "set_joint_values_right_arm_waist",
      std::bind(&SetJointValuesRightArmWaistServer::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
      std::bind(&SetJointValuesRightArmWaistServer::handle_cancel, this, std::placeholders::_1),
      std::bind(&SetJointValuesRightArmWaistServer::handle_accepted, this, std::placeholders::_1)
    );

    RCLCPP_INFO(this->get_logger(), "SetJointValuesRightArmWaistServer is up.");
  }
...
private:
  ...
  std::thread([this, goal_handle]() {
    ...
    moveit::planning_interface::MoveGroupInterface move_group(move_group_node_, "r_arm_waist_group");
...
int main(int argc, char **argv)
{
  ...
  auto action_node = std::make_shared<SetJointValuesRightArmWaistServer>(move_group_node);
